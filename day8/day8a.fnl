...
(fn antinode-coordinates [pt1 pt2]
  (let [distance (manhattan-distance pt1 pt2)
        antinode1 (add-manhattan-distance pt2 distance)
        antinode2 (add-manhattan-distance pt1 (negate distance))]
    [antinode1 antinode2]))

(print (fennel.view (antinode-coordinates [4 5] [6 6])))

(print (fennel.view (antinode-coordinates [6 6] [4 5])))
    ...
    (each [frequency points (pairs frequency-grid)]
      (when (>= (length points) 2)
        (each [pt1 pt2 (combine.combn points 2)]
          (let [antinodes (antinode-coordinates pt1 pt2)]
            (each [_ antinode (ipairs antinodes)]
              (when (in-bounds? antinode grid)
